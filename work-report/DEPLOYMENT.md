# Deployment Guide: Namecheap Subdomain (report.leip.co.in)

Since you are using a subdomain (`report.leip.co.in`), the app will be hosted at the **root** of that subdomain's folder.
...
2. Go to **Subdomains**.
3. Create the subdomain `report` for the domain `leip.co.in`.
4. Note the **Document Root** folder (usually something like `public_html/report` or just `report.leip.co.in`).
...
## 5. Done!
Your app should now be live at [report.leip.co.in](http://report.leip.co.in).
